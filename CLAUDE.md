...
- **`main`'s `tests/test_smoke.py` mutates real repo files (`data/screenshots/*.png`, `data/visual_qa_report.json`) on every run — known gap, not yet fixed on this branch.** `dev/multi-expert-improvements` already fixed this exact class of bug (see that branch's own gotcha entry, commit `964318e`: a `MACRO_SKIP_VISUAL_QA` env gate + a `test_no_real_file_mutation()` regression guard), but the fix was never ported to `main`. Until it is, always run `git status --short` after `python tests/test_smoke.py` on `main` and `git checkout -- data/screenshots/ data/visual_qa_report.json` to discard the mutation before committing anything else. Tracked as a follow-up task.
