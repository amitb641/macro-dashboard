...
- **`python tests/test_smoke.py` leaves the working tree dirty** — the snapshot/rollback test phase writes restored fixture data directly onto the real `data/state.json`, and `data/visual_qa_report.json` + all of `data/screenshots/*.png` also get modified during a full smoke run, even though the rest of the suite runs from an isolated `tmp_dir`. Confirmed via `git stash` + re-run against committed `HEAD`: reproduces on a clean tree with no code changes at all. After running smoke tests, always `git status --short` before staging anything — `git checkout -- data/screenshots/ data/state.json data/visual_qa_report.json` restores the committed versions without touching real changes elsewhere. A proper fix (redirect the snapshot/rollback path to the test's `tmp_dir`, same pattern `MACRO_STATE_FILE` already uses for the renderer test) is flagged as a follow-up, not yet done.
