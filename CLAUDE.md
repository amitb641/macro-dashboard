...
- **This "hardcoded pastel background in dark mode" bug is confirmed systemic — 5 instances found in one session** (Oil tab shock tracker; Fed Rates tab treasury-yield explainer panels; Oil tab's WTI/Brent/Crisis Range daily-summary tiles; the global nav icon SVGs; `.nav-btn`'s base state), found via 2 user screenshots, one direct user question, and one proactive `design:design-system` audit, **zero of the five caught by `visual_qa.py`'s 449 checks**. Sub-shapes seen: (a) text correctly uses `var(--text2)` but the wrapping background is a hardcoded light pastel with no `_isDark` branch; (b) *both* background AND text hardcoded with zero theme awareness (the daily-tiles case); (c) an SVG `fill="#HEX"` presentation attribute inside JS-generated markup, same root cause as (a)/(b) but in a namespace `grep`-for-inline-`style` sweeps won't catch — fixed via a CSS attribute-selector override (`:root[data-theme="dark"] .nav-icon [fill="#FEF2F2"]{fill-opacity:.16}`) rather than editing the 28 SVG strings directly, since `[attr="value"]` selectors work on SVG presentation attributes exactly like any other attribute; (d) a dark-mode override rule that sets `color` but not `background`/`border-color`, so the un-overridden properties silently fall through to the light-mode base rule — this one hid *behind* the icon-fill fix (fixing the icon exposed that its parent `.nav-btn` pill was still solid white) and was only found because the icon fix let it become visible. A reusable fix palette has emerged across all 5 fixes: `_mutedTxt = _isDark ? '#94A3B8' : '#64748B'`, `_valTxt = _isDark ? '#A8C4D8' : '#334E68'`, backgrounds go from a light hex to `rgba(<accent-rgb>,.06-.10)` translucent tints in dark. **Also recurring: an ad-hoc contrast-verification script using a naive `/[\d.]+/g` regex mis-parses hex literals** (drops A-F, e.g. `#94A3B8` → wrongly parsed as rgb(94,3,8)) — always use `parseInt(hex.substr(n,2),16)`.
- **A per-selector contrast patch can mask a token-level bug for an unknown amount of time — always check whether the "fix" is patching a symptom or the root cause.** `--muted` (light theme, `#7A8FA8`) computed 3.32:1 against a white card — WCAG fail at the 8-11px sizes it's used across **112 call sites** (`.kpi-lbl`, `.panel-sub`, `.m-lbl`, `.card-note`, `.dtable th`, source attributions, etc). One selector, `.kpi-cluster-hdr`, already had a `body.themed-sm` override forcing it to `--text2` instead, with a code comment explicitly documenting the 3.32:1 failure — but nobody had gone back and fixed the *token itself*, so the other 111 call sites kept failing, undetected, for however long that comment had been sitting there. `--dim` (`#94A3B8`, 2.56:1) had the same problem with zero patches at all. Found via a proactive `design:design-system` audit reading token values directly (`getComputedStyle` on `:root`) rather than screenshot-hunting individual components. Fixed at the token: `--muted`→`#5B6B80` (5.12-5.44:1), `--dim`→`#5F6F84` (4.83-5.13:1) — one `:root{}` edit instead of 112 component edits. **Lesson: when you find a per-selector dark/light-mode color override, check what it's overriding *to* and *from* — if the override exists because the base token fails contrast, the base token itself is very likely still failing everywhere else that references it directly.**
...
- **The entire dark-theme palette is one clean `:root[data-theme="dark"]{--token:#hex;...}` block — the entire light-theme palette is NOT.** This asymmetry only becomes visible when you go to make a matching palette-level change in both themes. Dark mode's colors are 100% token-driven (index.html ~1846) — editing that one block is sufficient, confirmed 2026-07-01 doing the "OLED noir" dark repaint (commit `ff41927`). Light mode's colors are ~90% hardcoded as *literal hex* directly in `theme-overlay.css` (read-only, external, the actual default `body.themed-sm` skin per the earlier "themed-sm is the real skin" gotcha) — **48+ literal occurrences of just `--border`/`--bg`/`--card`'s current hex values** across ~40 different component rules (hero, meta strip, macro-state card, KPI tiles, nav, panels, tables, sort buttons, sector cards, delta chips, readbar, footnote ladder, release calendar), none of them reading `var(--border)` etc. Editing the light `:root{}` token block alone would have **zero visible effect** on light mode — theme-overlay.css's `!important` literal values win every one of those 40+ rules regardless of what the token says. A "light-mode palette shift" of any kind (not just this specific ink-on-paper idea) means writing ~40+ individual `body.themed-sm <selector>{...!important}` overrides in index.html to beat theme-overlay.css's existing `!important` rules — the same established mirror-and-override pattern used everywhere else in this file, just at a much larger scale than any single component fix has needed so far. Scope this honestly before starting; it is not a token edit.
- **A JS-baked hex literal that was correctly matched to a token's value at fix-time silently goes stale the next time that token changes — grep for the OLD value across the whole file before considering a token edit done.** Doing the OLED-noir dark repaint (commit `ff41927`), the token edit alone (`--text2` `#A8C4D8`→`#B8B8B8` etc.) left 3 JS-baked `_isDark ? '#A8C4D8' : '#334E68'`-style ternaries (in the oil shock tracker and treasury explainer render functions, both written earlier the same day) still hardcoding the *old* `#A8C4D8`/`#F5F7FF` values — they'd have kept rendering the previous navy-tinted colors indefinitely since nothing routes them through `var()`. Caught by grepping the file for every old hex string before considering the token edit complete, not by visual inspection (both components still looked "fine" at a glance — the old blue-gray isn't dramatically different from the new neutral gray at a screenshot's resolution). **Rule: after any `:root[data-theme]` token value change, grep the whole file for the OLD hex string(s) — any surviving hit outside the token block itself is a component that silently didn't get the update.** Translucent `rgba()` tints tied to a token (not a literal duplicate of it) are the one case that's safe to leave alone — they composite against whatever's behind them and don't need updating.
