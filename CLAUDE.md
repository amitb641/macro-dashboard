...
- **This "hardcoded pastel background, theme-aware text" bug is a systemic pattern, not an isolated incident — found twice in one session (Oil tab shock tracker, Fed Rates tab treasury-yield explainer panels), both via user screenshots, both undiscovered by `visual_qa.py`'s 449 checks.** The shape is always the same: a JS-generated `innerHTML` block gives body text `color:var(--text2)` (correctly theme-aware) but wraps it in a `background:#F5F0FF`-style hardcoded light pastel with no `_isDark` branch — in dark mode, `--text2` resolves to a *light* color that lands on a background that never left light mode, producing light-on-light. `visual_qa.py` didn't catch either instance, meaning there are very likely more of these elsewhere in `index.html` (bank earnings cards, sector cards, other explainer/callout panels across other tabs) waiting to be found the same ad-hoc way. **A systematic grep for hardcoded light hex backgrounds (`#F[0-9A-F]{5}`-style very-light colors) combined with `var(--text` in the same block, across the whole file, would find these proactively instead of one screenshot at a time** — worth doing as a dedicated sweep, not deferred to the next accidental discovery.
