...
Distinct from snapshot.py (rolling, capped at 52, exists for operational
rollback) — this is the permanent archive: one directory per calendar
month, upserted on every pipeline run, NEVER pruned. Volume is tiny
(~2-3 MB/month of raw_data.json + index.html), so "keep forever" is the
right default at this scale — do not add a retention cap here without
...
Each month directory contains:
  manifest.json  — run metadata: dates, git sha/branch, validation status,
                   CEO-grade verdict, file sizes + sha256 checksums
  ...
  state.json     — Tier-1 chart payload (dev only; absent on main)
