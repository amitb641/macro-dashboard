...
# Gmail OAuth via Google service-account or installed-app token
# In GitHub Actions: store token JSON in secret GMAIL_TOKEN
...
def send_gmail(html_body, today_str):
    """Send via Gmail API using OAuth token stored in GMAIL_TOKEN secret."""
    ...
    try:
        ...
        print(f'  ⚠  GMAIL_TOKEN parse error: {e}'); return
    ...
    try:
        ...
        print(f'  ⚠  Gmail token refresh failed: {e}'); return

    # Build MIME message
    msg = MIMEMultipart('alternative')
    msg['Subject'] = f'📊 Macro Dashboard — {today_str}'
    msg['From']    = EMAIL_FROM or token_data.get('email', 'me')
    msg['To']      = EMAIL_TO
    ...
    try:
        ...
        if r.status_code in (200, 201):
            ...
            print(f'  ⚠  Gmail send failed: {r.status_code} {r.text[:120]}')
    except Exception as e:
...
def publish(snapshot_only=False):
    ...
    if not snapshot_only:
        ...
        send_gmail(html_body, today_str)
    else:
