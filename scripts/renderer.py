...
def inject_oil_daily(html, oil_daily):
    """Replace OIL_DAILY constant in HTML with fresh data from collector."""
    import json as _json
    new_val = _json.dumps(oil_daily, separators=(',', ':'))
    new_const = 'window.__OIL_DAILY__ || ' + new_val

    # Find and replace the OIL_DAILY fallback object in the HTML
    pat = re.compile(r'window\.__OIL_DAILY__\s*\|\|[^;]*?\}(?=\s*;)')
    new_html, n = pat.subn(new_const, html, count=1)
    if n:
        applied.append('OIL_DAILY (%d sessions, %s)' % (
            oil_daily.get('sessions', 0), oil_daily.get('month', '')))
    else:
        errors.append('inject_oil_daily: pattern not matched')
    return new_html
