...
5. ✅ **Agent 0 (pre-flight).** `scripts/preflight.py` runs as the first step in `briefing.yml`. Two checks:
   - **FRED series IDs** — extracts every series_id literal from `collector.py` and pings each one with `limit=1`. Any 4xx halts the pipeline before Agent 1 wastes a fetch. 700ms throttle keeps under FRED's 120 req/min limit.
   - **Anthropic model IDs** — fetches `GET https://api.anthropic.com/v1/models`, compares the served catalog against the IDs in `scripts/_models.py`. Catches deprecated aliases the moment Anthropic drops them. Was added after run #116 surfaced `claude-sonnet-4-6-20250514` returning 404 from Agent 3 — without this check, that class of bug only fails at runtime when each AI agent independently 404s.

   Defense-in-depth alongside Pass 3h: Agent 0 fails upstream; 3h surfaces anything that slipped through. Bypass for local dev: `PREFLIGHT_SKIP=1`.
